...
```{r freqtable, fig.cap="Frequency table for the **`speaker`** variable", echo=FALSE, fig.align="center", out.width="100%"}

knitr::include_graphics("img/mechanics/freqtable.png", error = TRUE)
...
```{r contingencyrow, fig.cap="Contingency table for the **`speaker`** and **`utterance`** variables, with row percentages", echo=FALSE, fig.align="center", out.width="100%"}

knitr::include_graphics("img/mechanics/contingencyrow.png", error = TRUE)

```

```{r contingencycol, fig.cap="Contingency table for the **`speaker`** and **`utterance`** variables, with column percentages", echo=FALSE, fig.align="center", out.width="100%"}

knitr::include_graphics("img/mechanics/contingencycol.png", error = TRUE)
...
Not only that, but as Table \@ref(tab:logicaltab) illustrates, there are several other logical operators that you can use corresponding to some basic mathematical concepts. Hopefully these are all pretty self-explanatory. For example, the [**less than**]{color="#F58231"} operator `<` checks to see if the number on the left is less than the number on the right. If it's less, then jamovi returns an answer of `TRUE`, but if the two numbers are equal, or if the one on the right is larger, then jamovi returns an answer of `FALSE`.


In contrast, the [**less than or equal to**]{color="#F58231"} operator `<=` will do exactly what it says. It returns a value of `TRUE` if the number of the left hand side is less than or equal to the number on the right hand side. At this point I hope it's pretty obvious what the [**greater than**]{color="#F58231"} operator `>` and the [**greater than or equal to**]{color="#F58231"} operator `>=` do! 

Next on the list of logical operators is the [**not equal to**]{color="#F58231"} operator `!=` which, as with all the others, does what it says it does. It returns a value of `TRUE` when things on either side are not identical to each other. Therefore, since $2+2$ isn't equal to $5$, we would get 'true' as the value for our newly computed variable. Try it and see:

`2 + 2 != 5`



```{r logicaltab, echo=FALSE}
knitr::kable(rbind(
...
knitr::kable(rbind(
                    c("not", "NOT", "NOT(1==1)", "`FALSE`"),
                    c("or", "or", "(1==1) or (2==3)", "`TRUE`"),
                    c("and", "and", "(1==1) and (2==3)", "`FALSE`")),
...
), caption = "The ordering of various text characters used by the < and > operators. Not shown is the “space” character, which actually comes ﬁrst on the list.")
```
...
`mathematical function` = c("square root", 
                            ...
                            "exponentiation", 
                            "box-cox"),
`R function` = c("SQRT(x)", "ABS(x)", "LOG10(x)", "LN(x)", "EXP(x)", "BOXCOX(x, lamda)"),
`example input` = c("SQRT(25)", "ABS(-23)", "LOG10(1000)", "LN(1000)", "EXP(6.908)", "BOXCOX(6.908, 3)"),
answer = c("5", "23", "3",  "6.908", "1000.245", "109.551")), caption = "Some of the mathematical functions available in jamovi")
```
...
In order to understand logarithms and exponentials, the easiest thing to do is to actually calculate them and see how they relate to other simple calculations. There are three jamovi functions in particular that I want to talk about, namely `LN()`, `LOG10()` and `EXP()`. To start with, let's consider `LOG10()`, which is known as the 'llogarithm in base 10'. The trick to understanding a [**logarithm**]{color="#F58231"} is to understand that it's basically the "opposite" of taking a power. Specifically, the logarithm in base 10 is closely related to the powers of 10. So let's start by noting that 10-cubed is 1000. Mathematically, we would write this:
$$ 
10^3 = 1000
$$
...
This section returns to the **`nightgarden.csv`** data set. If you're reading this whole chapter in one sitting, then you should already have this data set loaded into a jamovi window. For this section, let's focus on the two variables **`speaker`** and **`utterance`** (see Section \@ref(freqtables) if you've forgotten what those variables look like). Suppose that what I want to do is pull out only those utterances that were made by Makka-Pakka. To that end, we need to specify a filter in jamovi. First open up a filter window by clicking on 'Filters' on the main jamovi 'Data' toolbar. Then, in the 'Filter 1' text box, next to the '=' sign, type the following:

`speaker == 'makka-pakka'`

```{r subset1, fig.cap="Creating a subset of the `nightgarden` data using the jamovi 'Filters' option", echo=FALSE, fig.align="center", out.width="100%"}

knitr::include_graphics("img/mechanics/subset1.png", error = TRUE)

```

When you have done this, you will see that a new column has been added to the spreadsheet window (see Figure \@ref(fig:subset1)), labelled 'Filter 1', with the cases where **`speaker`** **is not** 'makka-pakka' greyed-out (i.e., filtered out) and, conversely, the cases where **`speaker`** **is** 'makka-pakka' have a green check mark indicating they are filtered in. You can test this by running 'Exploration' - 'Descriptives' - 'Frequency tables' for the **`speaker`** variable and seeing what that shows. Go on, try it!
