...
```{r fa1, fig.cap="Latent factor underlying the relationship between several observed variables", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/fa1.png", error = TRUE)
...
First, we need some data. Twenty-five personality self-report items (see Figure \@ref(fig:fa2)) taken from the International Personality Item Pool (http://ipip.ori.org) were included as part of the Synthetic Aperture Personality Assessment (SAPA) web-based personality assessment (SAPA: http://sapa-project.org) project. The 25 items are organized by five putative factors: Agreeableness, Conscientiousness, Extraversion, Neuroticism, and Openness. 

```{r fa2, fig.cap="Twenty-five observed variable items organised by five putative personality factors in the dataset **`bfi_sample.csv`**", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/fa2.png", error = TRUE)
...
```{r fa3, fig.cap="The jamovi EFA analysis window", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/fa3.png", error = TRUE)

```

```{r fa4, fig.cap="jamovi EFA assumption checks for the personality questionnaire data", out.width="100%", fig.show = "hold", echo = FALSE}
knitr::include_graphics("img/factoranalysis/fa4.png", error = TRUE)
...
- One convention is to choose all components with Eigen values greater than 1^[An Eigen value indicates how much of the variance in the observed variables a factor accounts for. A factor with an Eigen value $>1$ accounts for more variance than a single observed variable]. This would give us four factors with our data (try it and see). 
...
```{r fa5, fig.cap="Scree plot of the personality data in jamovi EFA, showing a noticeable inflection and levelling off after point 5 (the \"elbow\")", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/fa5.png", error = TRUE)

```

```{r fa6, fig.cap="Factor summary statistics and correlations for a five factor solution in jamovi EFA", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/fa6.png", error = TRUE)
...
```{r fa8, fig.cap="Recode variable using the jamovi Transform command", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/fa8.png", error = TRUE)

```

```{r fa9, fig.cap="Compute new scale score variable using the jamovi Computed variable command", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/fa9.png", error = TRUE)

```

Another option is to create an optimally-weighted factor score index. We can use the jamovi `Rj` editor to do this in `R`. Again, there are two steps:

- Use the `Rj` editor to run the EFA in `R` to the same specification as the one in jamovi (i.e. five factors and oblimin rotation) and compute optimally weighted factor scores. Save the new dataset, with the factor scores, to a file. See Figure \@ref(fig:fa10).
...
```{r fa10, fig.cap="`Rj` editor commands for creating optimally weighted factor scores for the five factor solution", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/fa10.png", error = TRUE)

```

```{r fa11, fig.cap="The newly created data file “bfifactscores.csv” created in the `Rj` editor and containing the five factor score variables. Note that each of the new factor score variables is labelled corresponding to the order that the factors are listed in the factor loadings table", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/fa11.png", error = TRUE)
...
```{r fa12, fig.cap="Comparing differences in Agreeableness factor-based scores between males and females", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/fa12.png", error = TRUE)
...
```{r pca1, fig.cap="The jamovi PCA analysis window", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/pca1.png", error = TRUE)
...
```{r pca2, fig.cap="jamovi PCA assumption checks for the personality item data", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/pca2.png", error = TRUE)
...
```{r pca3, fig.cap="Scree plot of the personality item data in jamovi PCA, showing the levelling off point, the “elbow”, after component 5", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/pca3.png", error = TRUE)
...
```{r pca4, fig.cap="Component summary statistics and correlations for a five component solution in jamovi PCA", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/pca4.png", error = TRUE)
...
```{r pca5, fig.cap="Component loadings for a five component solution in jamovi PCA", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/pca5.png", error = TRUE)
...
```{r cfa2, fig.cap="The jamovi CFA analysis window", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/cfa2.png", error = TRUE)

```

Once we have set up the analysis we can turn our attention to the jamovi results window and see what’s what. The first thing to look at is [**model fit**]{color="#F58231"} (Figure \@ref(fig:cfa3)) as this tells us how good a fit our model is to the observed data. NB in our model only the pre-specified covariances are estimated, including the factor correlations by default. Everything else is set to zero.

```{r cfa3, fig.cap="The jamovi CFA Model Fit results for our CFA model", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/cfa3.png", error = TRUE)
...
```{r cfa4, fig.cap="The jamovi CFA Factor Loadings table for our CFA model", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/cfa4.png", error = TRUE)

```

```{r cfa5, fig.cap="The jamovi CFA Factor Covariances table for our CFA model", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/cfa5.png", error = TRUE)

```

How could we improve the model? One option is to go back a few stages and think again about the items / measures we are using and how they might be improved or changed. Another option is to make some *post hoc* tweaks to the model to improve the fit. One way of doing this is to use “modification indices”, specified as an ‘Additional output’ option in jamovi (see Figure \@ref(fig:cfa6)).

```{r cfa6, fig.cap="The jamovi CFA Factor Loadings Modification Indices", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/cfa6.png", error = TRUE)

```

What we are looking for is the highest modification index (MI) value. We would then judge whether it makes sense to add that additional term into the model, using a *post hoc* rationalisation. For example, we can see in Figure \@ref(fig:cfa6) that the largest MI for the factor loadings that are not already in the model is a value of 28.786 for the loading of N4 (“Often feel blue”) onto the latent factor Extraversion. This indicates that if we add this path into the model then the chi-square value will reduce by around the same amount. 
...
```{r cfa7, fig.cap="The jamovi CFA Residual Covariances Modification Indices", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/cfa7.png", error = TRUE)
...
## Multi-Trait Multi-Method CFA{#MTMM}

In this section we're going to consider how different measurement techniques or questions can be an important source of data variability, known as [**method variance**]{color="#F58231"}. To do this, we'll use another psychological data set, one that contains data on "attributional style". 

The Attributional Style Questionnaire (ASQ) @Hewitt2004 collected psychological wellbeing data from young people in the United Kingdom and New Zealand. They measured attributional style for negative events, which is how people habitually explain the cause of bad things that happen to them [@Peterson1984]. The attributional style questionnaire (ASQ) measures three aspects of attributional style:
...
```{r MTMM1, fig.cap="The Attributional Style Questionnaire (ASQ) for negative events", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/MTMM1.png", error = TRUE)
...
```{r MTMM2, fig.cap="Six questions on the ASQ for each of the Internality, Stability and Globality dimensions", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/MTMM2.png", error = TRUE)

```

```{r MTMM3, fig.cap="Initial pre-specification of latent factor structure for the ASQ", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/MTMM3.png", error = TRUE)
...
```{r MTMM5, fig.cap="The jamovi CFA analysis window", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/MTMM5.png", error = TRUE)
...
```{r MTMM6, fig.cap="The jamovi CFA Model Fit results for our CFA MTMM model", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/MTMM6.png", error = TRUE)
...
What we are looking for is the highest modification index (MI) value. We would then judge whether it makes sense to add that additional term into the model, using a *post hoc* rationalisation. For example, we can see in Figure \@ref(fig:MTMM8) that the largest MI for the factor loadings that are not already in the model is a value of $24.52$ for the loading of INT6 onto the latent factor Globality. This indicates that if we add this path into the model then the chi-square value will reduce by about $25$. But in our model adding this path doesn't really make any theoretical or methodological sense, and therefore we won’t be including this path in a revised model.

```{r MTMM7, fig.cap="The jamovi CFA Factor Loadings and Covariances tables for our CFA MTMM model", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/MTMM7.png", error = TRUE)
...
```{r MTMM8, fig.cap="The jamovi CFA Factor Loadings Modification Indices", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/MTMM8.png", error = TRUE)

```

```{r MTMM9, fig.cap="The jamovi CFA Residual Covariances Modification Indices", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/MTMM9.png", error = TRUE)

```

## Internal consistency reliability analysis{#rel}
...
```{r rel1, fig.cap="The jamovi Reliability Analysis window", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/rel1.png", error = TRUE)
...
- Under the “Reverse Scaled Items” option, select variable **`A11** in the “Normal Scaled Items” box and move it across to the “Reverse Scaled Items” box. 
...
```{r rel2, fig.cap="The jamovi Reliability Analysis results for the Agreeableness factor", echo=FALSE, fig.align="center", out.width="100%", fig.show = "hold"}
knitr::include_graphics("img/factoranalysis/rel2.png", error = TRUE)
